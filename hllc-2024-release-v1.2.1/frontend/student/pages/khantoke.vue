              ...
              <div v-if="activity" class="pa-6 border rounded-xl">
                <div class="pb-6">
                  <ActivityDotTracking
                    ...
                    :show-window="true"
                  >
                    <template #[`title.checkin`]>
                      {{ lang === 'en' ? 'Recieve Ticket' : 'รับตั๋วขันโตก' }}
                    </template>
                  </ActivityDotTracking>
                </div>
                <v-window v-model="activeWindow">
                  ...
                  <v-window-item value="checkin">
                    ...
                    <v-btn
                      ...
                      >
                      <span class="text-white">{{ textMessage.qrButton[lang] }}</span>
                    </v-btn>
                  </v-window-item>
                  ...
                  <v-window-item value="assessment">
                    ...
                    <v-btn
                      ...
                      ><span class="text-white">{{ textMessage.evaluationButton[lang] }}</span></v-btn
                    >
                  </v-window-item>
                </v-window>
              </div>
...
const {
  ...
  status,
  error,
} = await useAsyncData<IActivity>(`khantoke`, async () => {
  const response = await $api.get<ApiResponse<IActivity>>(
    `/activities/KHANTOKE/code`
  )
...
const textMessage = {
  ...
  checkinFailed: {
    en: `You did not participate this activity`,
    th: 'คุณไม่ได้เข้าร่วมกิจกรรมนี้',
  },
  checkedin: {
    en: `You have been recieved Khantoke ticket at ${formattedDateTime.value?.checkinTime.en}.`,
    th: `คุณได้ลงทะเบียนรับตั๋วขันโตกเมื่อ ${formattedDateTime.value?.checkinTime.th} น.`,
  },
  ongoing: {
...
const { init, questions, save } = useAsssessment(activity.value!)
